...
def generar(datos):
    ...
    for i in range(len(t0.rows) - 1, n_prod, -1):
        t0.rows[i]._tr.getparent().remove(t0.rows[i]._tr)
    ...
    # ── 4. Traslado y Mano de obra ──
    # Indices fijos: [9]=título, [11]=tabla
    body = list(doc.element.body)
    titulo_tras = body[9]   # "Servicio Adicional: Traslado y Mano de obra"
    tabla_tras  = body[11]  # tabla de traslado
    t1 = doc.tables[1]
    ...
    if inc_tras or inc_mano:
        ...
        if inc_tras and p_tras > 0:
            ...
            for col in [2, 4]:
                ...
                if target_p.runs:
                    target_p.runs[0].text = f'S/. {p_tras:,.2f}'
                    for r in target_p.runs[1:]: r.text = ''
                else: target_p.add_run(f'S/. {p_tras:,.2f}')
        else:
